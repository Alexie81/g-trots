...
export default function ShopInventoryManager() {
  ...
  return <View style={styles.wrap}>
    ...
    {filtered.length ? pagedProducts.map((product) => {
      ...
      const canAdjust = product.stock_mode === 'tracked' && !isSupplierManaged;
      return <TouchableOpacity key={product.id} style={styles.card} activeOpacity={canAdjust ? 0.76 : 1} onPress={() => canAdjust && void open(product)}>
        ...
        {canAdjust ? <ChevronRight size={18} color={Colors.textMuted} /> : <Boxes size={19} color={isSupplierManaged ? Colors.orange : '#22C55E'} />}
      </TouchableOpacity>;
