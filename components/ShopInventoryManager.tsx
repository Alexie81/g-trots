...
import ShopInventoryExportButton from '@/components/ShopInventoryExportButton';
...
export default function ShopInventoryManager({ onOpenNir, header, bottomInset = 0 }: { onOpenNir?: (nirId: string) => void; header?: React.ReactNode; bottomInset?: number }) {
  const { token } = useAuth();
  ...
  return <><ScrollView style={styles.scroll} contentContainerStyle={{ paddingBottom: 94 + bottomInset }} stickyHeaderIndices={[1]} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
    <View style={styles.pageHeader}>{header}</View>
    <View style={styles.actions}><View style={styles.actionsRow}><View style={styles.totalPill}><Text style={styles.totalPillText}>{filtered.length}</Text></View><View style={styles.search}><Search size={17} color={Colors.textMuted} /><TextInput value={query} onChangeText={(value) => { setQuery(value); setPage(1); }} placeholder="Nume, cod, furnizor sau marcă" placeholderTextColor={Colors.textMuted} style={styles.searchInput} />{query ? <TouchableOpacity accessibilityLabel="Șterge căutarea" style={styles.clearSearch} onPress={clearSearch}><X size={16} color={Colors.textSecondary} /></TouchableOpacity> : null}</View><ShopInventoryExportButton /><TouchableOpacity style={styles.refresh} onPress={() => void load()}><RefreshCw size={18} color={Colors.textSecondary} /></TouchableOpacity></View></View>
      ...
      <View style={styles.backdrop}><SafeAreaView style={styles.sheet} edges={['top', 'bottom']}><View style={styles.sheetHeader}><View style={styles.sheetHeaderCopy}><Text style={styles.sheetKicker}>STOCURI / FIȘA PRODUSULUI</Text><Text style={styles.productCodeTop}>{selected?.sku || 'FĂRĂ COD'}</Text><AutoScrollProductName value={selected?.name || ''} style={styles.sheetTitle} /><Text style={styles.sheetSubtitle}>Toate intrările și mișcările acestui produs, explicate cronologic.</Text></View><TouchableOpacity style={styles.close} onPress={() => setSelected(null)}><X size={20} color={Colors.textSecondary} /></TouchableOpacity></View>
        <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={styles.sheetContent}>
          ...
          <View style={styles.ledgerSection}><View style={styles.ledgerHeader}><View style={styles.ledgerIconGreen}><ArrowDownLeft size={19} color="#4ADE80" /></View><View style={styles.ledgerHeaderCopy}><Text style={styles.ledgerKicker}>DOCUMENTELE CARE AU ADĂUGAT MARFĂ</Text><Text style={styles.ledgerTitle}>Intrări din NIR-uri</Text><Text style={styles.ledgerDescription}>Apasă pe o intrare ca să deschizi nota de recepție aferentă.</Text></View><View style={styles.countBadge}><Text style={styles.countBadgeText}>{nirEntries.length}</Text></View></View>
            ...
            <View style={styles.futureNote}><Text style={styles.futureNoteText}>Ieșirile din facturile emise sunt detaliate mai jos. Proveniența este stabilită automat din loturile de achiziție, iar costul afișat este costul real din fiecare NIR.</Text></View>
          </View>
          ...
          <View style={styles.ledgerSection}><View style={styles.ledgerHeader}><View style={styles.ledgerIconBlue}><History size={19} color="#93C5FD" /></View><View style={styles.ledgerHeaderCopy}><Text style={styles.ledgerKicker}>JURNAL CRONOLOGIC COMPLET</Text><Text style={styles.ledgerTitle}>Mișcări de stoc</Text><Text style={styles.ledgerDescription}>Vezi ce s-a schimbat și ce stoc a rămas după fiecare operațiune.</Text></View><View style={styles.countBadge}><Text style={styles.countBadgeText}>{movements.length}</Text></View></View>
            {movementsLoading ? <View style={styles.ledgerLoading}><ActivityIndicator color="#93C5FD" /></View> : movementRows.length ? movementRows.map((movement) => {
              const delta = movementDelta(movement); const incoming = delta >= 0; const fifoPending = Number(movement.fifo_quantity_pending || 0);
              return <View key={movement.id} style={styles.movementCard}><View style={[styles.movementDirection, incoming ? styles.directionIn : styles.directionOut]}>{incoming ? <ArrowDownLeft size={17} color="#4ADE80" /> : <ArrowUpRight size={17} color="#FCA5A5" />}</View><View style={styles.movementCopy}><Text style={styles.movementTitle}>{movementLabel(movement.movement_type)}</Text><Text numberOfLines={2} style={styles.movementNote}>{(movement.note || 'Mișcare înregistrată automat').replace(/FIFO/gi, 'lot')}</Text>{fifoPending > 0 ? <View style={styles.fifoPending}><Text style={styles.fifoPendingTitle}>COST DE ACHIZIȚIE NEASOCIAT · {fifoPending.toLocaleString('ro-RO')} buc.</Text><Text style={styles.fifoPendingText}>Factura este emisă; furnizorul și costul de achiziție se completează automat la confirmarea NIR-ului.</Text></View> : null}{movement.fifo_allocations?.map((allocation) => <View key={allocation.layer_id} style={styles.fifoAllocation}><Text style={styles.fifoAllocationTitle}>LOT DE ACHIZIȚIE ASOCIAT · {Number(allocation.quantity).toLocaleString('ro-RO')} buc. · {allocation.supplier_display_name}</Text><Text style={styles.fifoAllocationMeta}>{allocation.nir_number || allocation.invoice_number_snapshot || allocation.source_reference || 'Lot inițial'} · {movementDate(allocation.reception_date)} · cost achiziție {moneyRon(allocation.unit_cost_ron)}/buc.</Text></View>)}<Text style={styles.movementDate}>{movementDate(movement.created_at)}</Text></View><View style={styles.movementNumbers}><Text style={[styles.movementDelta, incoming ? styles.valuePositive : styles.valueNegative]}>{incoming ? '+' : ''}{delta.toLocaleString('ro-RO')}</Text><Text style={styles.movementAfter}>rămân {movementAfter(movement).toLocaleString('ro-RO')} buc.</Text>{movement.sale_unit_price_ron != null ? <Text style={styles.movementSalePrice}>vândut cu {moneyRon(movement.sale_unit_price_ron)}</Text> : null}</View></View>;
...
const styles = StyleSheet.create({
  scroll: { flex: 1 }, pageHeader: { paddingHorizontal: 16 }, wrap: { paddingHorizontal: 16, paddingTop: 6 }, state: { minHeight: 260, alignItems: 'center', justifyContent: 'center', gap: 12, borderRadius: 24, backgroundColor: '#1B1B1F', marginTop: 16 }, stateText: { color: Colors.textSecondary, fontFamily: 'Inter-Regular', fontSize: 11 }, error: { color: '#FCA5A5', fontFamily: 'Inter-Regular', fontSize: 11, textAlign: 'center', paddingHorizontal: 22 }, retry: { borderRadius: 999, paddingHorizontal: 14, paddingVertical: 9, backgroundColor: Colors.orangeDim }, retryText: { color: Colors.orange, fontFamily: 'Inter-SemiBold', fontSize: 10 },
  actions: { width: '100%', height: 58, borderBottomWidth: 1, borderBottomColor: '#302C33', backgroundColor: Colors.bg, elevation: 14 }, actionsRow: { position: 'absolute', left: 0, right: 0, top: 0, bottom: 0, flexDirection: 'row', flexWrap: 'nowrap', alignItems: 'center', gap: 5, paddingHorizontal: 8, paddingVertical: 7 }, totalPill: { width: 42, height: 42, flexShrink: 0, alignItems: 'center', justifyContent: 'center', borderRadius: 13, backgroundColor: '#211F24' }, totalPillText: { color: Colors.textPrimary, fontFamily: 'Inter-Bold', fontSize: 10 }, search: { flex: 1, minWidth: 0, height: 42, flexDirection: 'row', flexWrap: 'nowrap', alignItems: 'center', gap: 6, borderRadius: 13, paddingHorizontal: 10, backgroundColor: '#1B1B1F' }, searchInput: { flex: 1, minWidth: 0, color: Colors.textPrimary, fontFamily: 'Inter-Regular', fontSize: 10 }, clearSearch: { width: 28, height: 28, alignItems: 'center', justifyContent: 'center', borderRadius: 9, backgroundColor: '#2B282E' }, refresh: { width: 42, height: 42, flexShrink: 0, alignItems: 'center', justifyContent: 'center', borderRadius: 13, backgroundColor: '#1B1B1F' },
  ...
  backdrop: { flex: 1, justifyContent: 'flex-end', backgroundColor: 'rgba(0,0,0,0.82)' }, sheet: { height: '96%', borderTopLeftRadius: 28, borderTopRightRadius: 28, backgroundColor: '#17161A', overflow: 'hidden' }, sheetHeader: { minHeight: 112, flexDirection: 'row', alignItems: 'center', gap: 12, paddingHorizontal: 18, paddingVertical: 15, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: '#37333B', backgroundColor: '#1D1B20' }, sheetHeaderCopy: { flex: 1, minWidth: 0 }, sheetKicker: { color: '#5EEAD4', fontFamily: 'Inter-Bold', fontSize: 7, letterSpacing: 1 }, productCodeTop: { color: '#5EEAD4', fontFamily: 'Inter-Bold', fontSize: 24, lineHeight: 29, marginTop: 5 }, sheetTitle: { color: Colors.textPrimary, fontFamily: 'Inter-Bold', fontSize: 12, lineHeight: 17, marginTop: 2 }, sheetSubtitle: { color: Colors.textMuted, fontFamily: 'Inter-Regular', fontSize: 8, marginTop: 4 }, close: { width: 40, height: 40, alignItems: 'center', justifyContent: 'center', borderRadius: 14, backgroundColor: '#2B292F' }, sheetContent: { padding: 14, paddingBottom: 32 },
