...
function Pager({ page, total, onChange }: { page: number; total: number; onChange: (page: number) => void }) {
  ...
  return <View style={styles.ledgerPager}>
    <Text style={styles.ledgerRange}>{(page - 1) * LEDGER_PAGE_SIZE + 1}–{Math.min(total, page * LEDGER_PAGE_SIZE)} din {total}</Text>
    <View style={styles.ledgerPages}>
      ...
      {pages.map((value) => <TouchableOpacity key={value} style={[styles.pageNumber, page === value && styles.pageNumberActive]} onPress={() => onChange(value)}><Text style={[styles.pageNumberText, page === value && styles.pageNumberTextActive]}>{value}</Text></TouchableOpacity>)}
