...
function InvoicePreview({ palette }: { palette: ShopInvoiceThemePalette }) {
  return <View style={styles.paper}>
    ...
    <View style={styles.paperHead}>
      <View style={[styles.paperLogo, { backgroundColor: palette.accent }]}><Text style={styles.paperLogoText}>GT</Text></View>
      <View style={styles.paperTitle}><View style={styles.paperLineStrong} /><View style={styles.paperLineShort} /></View>
      <View style={styles.paperNumber}><View style={[styles.paperTinyAccent, { backgroundColor: palette.accent_dark }]} /><View style={styles.paperLineMini} /></View>
    </View>
    <View style={[styles.paperStatus, { backgroundColor: palette.soft, borderColor: palette.accent }]}><View style={[styles.paperPill, { backgroundColor: palette.accent }]} /><View style={[styles.paperStatusLine, { backgroundColor: `${palette.accent_dark}66` }]} /></View>
    <View style={styles.paperParties}><View style={[styles.paperParty, { borderTopColor: palette.accent }]}><View style={styles.paperLineStrong} /><View style={styles.paperLine} /><View style={styles.paperLineShort} /></View><View style={styles.paperParty}><View style={styles.paperLineStrong} /><View style={styles.paperLine} /><View style={styles.paperLineShort} /></View></View>
    <View style={styles.paperTable}><View style={styles.paperTableHead} />{[0, 1, 2].map((row) => <View key={row} style={styles.paperTableRow}><View style={[styles.paperThumb, { backgroundColor: palette.soft }]} /><View style={styles.paperCellLines}><View style={styles.paperLineStrong} /><View style={styles.paperLineShort} /></View><View style={[styles.paperAmount, { backgroundColor: `${palette.accent}35` }]} /></View>)}</View>
    <View style={styles.paperFooter}><View style={styles.paperFooterLines}><View style={styles.paperLine} /><View style={styles.paperLineShort} /></View><View style={[styles.paperTotal, { backgroundColor: palette.accent }]} /></View>
  </View>;
}
      ...
      <View style={styles.themeGrid}>
        {themeOrder.map((theme) => {
          ...
          return <TouchableOpacity key={theme} activeOpacity={0.88} accessibilityRole="radio" accessibilityState={{ checked: isSelected }} onPress={() => setSelected(theme)} style={[styles.themeCard, wide && styles.themeCardWide, isSelected && { borderColor: palette.accent, backgroundColor: `${palette.accent}10` }]}>
            <View style={styles.previewWrap}><InvoicePreview palette={palette} /><View style={[styles.previewGlow, { backgroundColor: `${palette.accent}18` }]} /></View>
...
const styles = StyleSheet.create({
  ...
  paper: { zIndex: 1, minHeight: 190, overflow: 'hidden', borderRadius: 9, padding: 10, backgroundColor: '#FFF', shadowColor: '#000', shadowOpacity: .16, shadowRadius: 9, elevation: 2 }, paperAccent: { height: 4, marginHorizontal: -10, marginTop: -10, marginBottom: 8 }, paperHead: { height: 28, flexDirection: 'row', alignItems: 'center', gap: 6 }, paperLogo: { width: 24, height: 24, alignItems: 'center', justifyContent: 'center', borderRadius: 6 }, paperLogoText: { color: '#FFF', fontSize: 8, fontWeight: '900' }, paperTitle: { flex: 1, gap: 3 }, paperNumber: { width: 35, alignItems: 'flex-end', gap: 3 }, paperTinyAccent: { width: 28, height: 4, borderRadius: 2 }, paperLineStrong: { width: '72%', height: 3, borderRadius: 2, backgroundColor: '#312E32' }, paperLine: { width: '88%', height: 2, borderRadius: 2, backgroundColor: '#C9C5CA' }, paperLineShort: { width: '48%', height: 2, borderRadius: 2, backgroundColor: '#D7D3D7' }, paperLineMini: { width: 22, height: 2, borderRadius: 2, backgroundColor: '#D5D1D5' },
  paperStatus: { height: 19, marginTop: 7, flexDirection: 'row', alignItems: 'center', gap: 7, borderWidth: 1, borderRadius: 5, paddingHorizontal: 5 }, paperPill: { width: 25, height: 7, borderRadius: 5 }, paperStatusLine: { width: 45, height: 3, borderRadius: 2 }, paperParties: { marginTop: 7, flexDirection: 'row', gap: 5 }, paperParty: { height: 36, flex: 1, gap: 4, borderWidth: 1, borderColor: '#E1DEE2', borderTopWidth: 3, borderTopColor: '#29262B', borderRadius: 5, padding: 5 },
  paperTable: { marginTop: 7, overflow: 'hidden', borderWidth: 1, borderColor: '#E0DDE0', borderRadius: 5 }, paperTableHead: { height: 11, backgroundColor: '#29262B' }, paperTableRow: { height: 25, flexDirection: 'row', alignItems: 'center', gap: 5, borderTopWidth: 1, borderTopColor: '#ECEAEC', paddingHorizontal: 4 }, paperThumb: { width: 17, height: 17, borderRadius: 4 }, paperCellLines: { flex: 1, gap: 3 }, paperAmount: { width: 30, height: 6, borderRadius: 3 }, paperFooter: { marginTop: 7, flexDirection: 'row', alignItems: 'flex-end', gap: 9 }, paperFooterLines: { flex: 1, gap: 4 }, paperTotal: { width: 63, height: 17, borderRadius: 5 },
