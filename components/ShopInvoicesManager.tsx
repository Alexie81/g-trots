...
import { ArrowLeft, BadgeCheck, Building2, ChevronRight, Clock3, CloudUpload, Download, FileCheck2, FileCode2, FileSpreadsheet, Link2, Mail, MapPin, Package, Percent, RefreshCw, Search, Send, ShieldCheck, ShoppingBag, Trash2, Truck, X } from 'lucide-react-native';
...
export default function ShopInvoicesManager({ initialInvoiceId = null, onInitialInvoiceHandled, onOpenSpv }: { initialInvoiceId?: string | null; onInitialInvoiceHandled?: () => void; onOpenSpv?: () => void }) {
  ...
  const prepareSpv = (invoice: ShopIssuedInvoice) => {
    if (invoice.spv_status === 'sent') return;
    const openSettings = () => {
      dismissInvoice();
      setTimeout(() => onOpenSpv?.(), 240);
    };
    Alert.alert(
      'Conexiune SPV necesară',
      `${invoice.display_number} este pregătită pentru transmitere. Conectează mai întâi firma la ANAF din configurarea SPV.`,
      onOpenSpv ? [{ text: 'Mai târziu', style: 'cancel' }, { text: 'Configurează SPV', onPress: openSettings }] : [{ text: 'Am înțeles' }],
    );
  };
