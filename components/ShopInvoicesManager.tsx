...
export default function ShopInvoicesManager({ initialInvoiceId = null, onInitialInvoiceHandled, onOpenSpv }: { initialInvoiceId?: string | null; onInitialInvoiceHandled?: () => void; onOpenSpv?: () => void }) {
  ...
  return <><ScrollView style={styles.screen} contentContainerStyle={[styles.content, { paddingBottom: 105 + insets.bottom }]} showsVerticalScrollIndicator={false}>
    ...
    <View style={[styles.toolbar, { flexDirection: 'row', alignItems: 'center', gap: 9 }]}><View style={[styles.search, { flex: 1 }]}><Search size={17} color={Colors.textMuted} /><TextInput value={query} onChangeText={(value) => { setQuery(value); setPage(1); }} placeholder="Număr, comandă, client, CUI sau e-mail" placeholderTextColor={Colors.textMuted} style={styles.searchInput} /></View><ShopRegistryExportButton kind="invoices" total={invoices.length} /></View>
