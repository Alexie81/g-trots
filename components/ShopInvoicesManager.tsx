...
import { ArrowLeft, BadgeCheck, Building2, ChevronRight, Clock3, Download, FileCheck2, FileCode2, FileSpreadsheet, Link2, Mail, MapPin, Package, Percent, RefreshCw, Search, Send, ShieldCheck, ShoppingBag, Trash2, Truck, X } from 'lucide-react-native';
...
export default function ShopInvoicesManager({ initialInvoiceId = null, onInitialInvoiceHandled }: { initialInvoiceId?: string | null; onInitialInvoiceHandled?: () => void }) {
  const { token } = useAuth();
  ...
  return <><ScrollView style={styles.screen} contentContainerStyle={[styles.content, { paddingBottom: 105 + insets.bottom }]} showsVerticalScrollIndicator={false}>
    ...
    {loading ? <View style={styles.state}><ActivityIndicator color={Colors.orange} /><Text style={styles.stateText}>Se încarcă facturile emise...</Text></View> : error ? <View style={styles.state}><Text style={styles.error}>{error}</Text><TouchableOpacity style={styles.retry} onPress={() => void load()}><Text style={styles.retryText}>Încearcă din nou</Text></TouchableOpacity></View> : paged.length ? paged.map((invoice) => {
      ...
      return <LinearGradient key={invoice.id} colors={[`${accent}18`, '#201E22', '#19181B']} locations={[0, 0.38, 1]} style={[styles.card, { borderColor: `${accent}66` }]}>
        ...
        <TouchableOpacity activeOpacity={0.78} style={styles.cardMain} onPress={() => void openInvoice(invoice)}>
          ...
          <View style={styles.cardCopy}>
            <View style={styles.cardTop}>
              ...
              <View style={[styles.spvBadge, invoice.spv_status === 'sent' ? styles.spvSent : styles.spvPending]}><ShieldCheck size={11} color={invoice.spv_status === 'sent' ? '#6EE7B7' : '#FDA4AF'} /><Text style={[styles.spvText, { color: invoice.spv_status === 'sent' ? '#6EE7B7' : '#FDA4AF' }]}>SPV {invoice.spv_status === 'sent' ? 'TRIMISĂ' : 'NETRIMISĂ'}</Text></View>
            </View>
        ...
        <TouchableOpacity activeOpacity={0.78} style={[styles.cardSummary, { backgroundColor: `${accent}0D`, borderColor: `${accent}35` }]} onPress={() => void openInvoice(invoice)}>
          <View><Text style={styles.totalLabel}>TOTAL FACTURĂ</Text><Text style={[styles.total, { color: accent }]}>{money(invoice.total, invoice.currency)}</Text></View>
          <View style={[styles.openInvoiceButton, { backgroundColor: accent }]}><Text style={styles.openInvoiceButtonText}>Deschide fișa</Text><ChevronRight size={18} color="#FFFFFF" /></View>
        </TouchableOpacity>
      </LinearGradient>;
