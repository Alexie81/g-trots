...
export default function ShopNirManager({ initialNirId = null, onInitialNirHandled }: { initialNirId?: string | null; onInitialNirHandled?: () => void }) {
  ...
  const addSupplier = async () => {
    if (!token || !newSupplier?.alias.trim() || !newSupplier.name.trim()) return;
    setSaving(true);
    try {
      if (newSupplier.cui.trim()) {
        const duplicate = await shopApi.checkSupplierCui(token, newSupplier.cui);
        if (duplicate.supplier) {
          patchEditor({ supplier_id: duplicate.supplier.id, supplier_name: duplicate.supplier.name, supplier_alias: duplicate.supplier.alias });
          setNewSupplier(null);
      ...
      const supplier = await shopApi.createSupplier(token, {
        name: newSupplier.name.trim(), alias: newSupplier.alias.trim(), cui: newSupplier.cui.trim(), contact_person: '', email: '', phone: '', website: '',
        registration_number: '', address: '', notes: '', is_active: true,
      });
      setSuppliers((current) => [supplier, ...current]);
      patchEditor({ supplier_id: supplier.id, supplier_name: supplier.name, supplier_alias: supplier.alias });
      setNewSupplier(null);
      setSupplierPicker(false);
    } catch (error) {
  ...
  const filteredSuppliers = useMemo(() => suppliers.filter((supplier) => `${supplier.alias || ''} ${supplier.name} ${supplier.cui || ''}`.toLowerCase().includes(supplierSearch.toLowerCase())), [supplierSearch, suppliers]);
      ...
      <View style={styles.screen}>
        ...
        <ScrollView contentContainerStyle={[styles.editorContent, { paddingBottom: 166 + insets.bottom }]} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          ...
          <Reveal delay={20}><View style={styles.editorStepCard}>
            ...
            <TouchableOpacity style={styles.selector} disabled={!editable} onPress={() => setSupplierPicker(true)}><View><Text style={styles.label}>FURNIZOR *</Text><Text style={editor.supplier_id ? styles.selectorValue : styles.placeholder}>{shopSupplierDisplayName(editor, 'Selectează sau creează furnizor')}</Text></View>{editable && <ChevronRight size={19} color={Colors.textMuted} />}</TouchableOpacity>
...
function NirLineCard({ line, index, editable, supplierName, currency, exchangeRate, onPatch, onSupplierCodeChange, onSupplierNameChange, onPickProduct, onRemove }: { line: ShopNirLine; index: number; editable: boolean; supplierName: string; currency: string; exchangeRate: string; onPatch: (patch: Partial<ShopNirLine>) => void; onSupplierCodeChange: (value: string) => void; onSupplierNameChange: (value: string) => void; onPickProduct: () => void; onRemove: () => void }) {
  ...
  return <Reveal delay={Math.min(index * 45, 220)}><View style={[styles.lineCard, matched && styles.lineCardMatched]}>
    ...
    <LineStage number="3" title="Completează costul" subtitle={`Preț în ${currency}, TVA și costuri suplimentare`} icon={<CircleDollarSign size={17} color="#F59E0B" />}>
      ...
      <View style={styles.grid2}><Field label="TVA %" value={line.vat_rate} onChangeText={(vat_rate) => onPatch({ vat_rate })} placeholder="21" editable={editable} keyboardType="decimal-pad" /><Field label="COST SUPLIMENTAR RON" value={line.allocated_cost_ron || '0'} onChangeText={(allocated_cost_ron) => onPatch({ allocated_cost_ron })} placeholder="0" editable={editable} keyboardType="decimal-pad" /></View>
      <View style={styles.grid2}><Field label={`VALOARE FĂRĂ TVA · ${currency}`} value={editable && ['line_net', 'line_vat'].includes(line.price_entry_mode || '') ? line.line_net || '0' : editableDecimal(calculated.net)} onChangeText={(line_net) => onPatch({ line_net, price_entry_mode: 'line_net' })} placeholder="0.00" editable={editable} keyboardType="decimal-pad" /><Field label={`VALOARE TVA · ${currency}`} value={editable && line.price_entry_mode === 'line_vat' ? line.line_vat || '0' : editableDecimal(calculated.vat)} onChangeText={(line_vat) => onPatch({ line_net: editableDecimal(calculated.net, 6), line_vat, price_entry_mode: 'line_vat' })} placeholder="0.00" editable={editable} keyboardType="decimal-pad" /></View>
      <Field label={`VALOARE CU TVA · ${currency}`} value={editable && line.price_entry_mode === 'line_gross' ? line.line_total || '0' : editableDecimal(calculated.total)} onChangeText={(line_total) => onPatch({ line_total, price_entry_mode: 'line_gross' })} placeholder="0.00" editable={editable} keyboardType="decimal-pad" />
