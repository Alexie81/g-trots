      ...
      <View style={styles.screen}>
        ...
        <Modal visible={reverseDialog} transparent animationType="fade" statusBarTranslucent onRequestClose={() => !reversing && setReverseDialog(false)}>
          <Pressable style={styles.deleteBackdrop} onPress={() => !reversing && setReverseDialog(false)}>
            <Pressable style={[styles.deleteDialog, styles.reverseDialog, styles.stornoDialog]} onPress={(event) => event.stopPropagation()}>
              <View style={[styles.deleteDialogAccent, styles.reverseDialogAccent]} />
              <ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false} contentContainerStyle={styles.stornoDialogContent}>
                <View style={styles.stornoTitleRow}><View style={[styles.deleteDialogIcon, styles.reverseDialogIcon, styles.stornoTitleIcon]}><RotateCcw size={27} color="#FDA4AF" /></View><View style={{ flex: 1 }}><Text style={styles.deleteDialogEyebrow}>STORNARE CONTABILĂ</Text><Text style={styles.stornoDialogTitle}>Stornare factură</Text><Text style={styles.deleteDialogMessage}>Alege exact produsele și cantitățile care trebuie scoase din această recepție.</Text></View></View>
                <View style={styles.deleteDialogDocument}><View style={{ flex: 1 }}><Text style={styles.deleteDialogMetaLabel}>DOCUMENT</Text><Text numberOfLines={1} style={styles.deleteDialogMetaValue}>{editor.nir_number || editor.temporary_number || 'NIR'}</Text></View><View style={styles.deleteDialogDivider} /><View style={{ flex: 1 }}><Text style={styles.deleteDialogMetaLabel}>FURNIZOR</Text><Text numberOfLines={1} style={styles.deleteDialogMetaValue}>{editor.supplier_name || 'Necompletat'}</Text></View></View>

                <View style={styles.stornoOriginalInvoice}><View style={styles.stornoOriginalInvoiceHead}><View style={styles.stornoOriginalInvoiceIcon}><FileDown size={18} color="#7DD3FC" /></View><View><Text style={styles.stornoOriginalInvoiceEyebrow}>DOCUMENT DE REFERINȚĂ</Text><Text style={styles.stornoOriginalInvoiceTitle}>Factura originală</Text></View></View><View style={styles.stornoOriginalInvoiceFacts}><View style={styles.stornoOriginalInvoiceFact}><Text style={styles.stornoOriginalInvoiceLabel}>SERIE / NUMĂR</Text><Text numberOfLines={1} style={styles.stornoOriginalInvoiceValue}>{[editor.supplier_invoice_series, editor.supplier_invoice_number].filter(Boolean).join(' / ') || '—'}</Text></View><View style={styles.stornoOriginalInvoiceFact}><Text style={styles.stornoOriginalInvoiceLabel}>DATA</Text><Text style={styles.stornoOriginalInvoiceValue}>{editor.supplier_invoice_date || '—'}</Text></View><View style={styles.stornoOriginalInvoiceFact}><Text style={styles.stornoOriginalInvoiceLabel}>VALOARE</Text><Text style={[styles.stornoOriginalInvoiceValue, styles.stornoOriginalInvoiceAmount]}>{editor.grand_total !== undefined ? money(editor.grand_total, editor.currency || 'RON') : money(editor.grand_total_ron, 'RON')}</Text></View></View></View>

                <View style={styles.stornoInvoiceFields}><View style={styles.stornoInvoiceField}><Text style={styles.reverseReasonLabel}>SERIE FACTURĂ STORNO (OPȚIONAL)</Text><View style={styles.stornoInvoiceInputWrap}><TextInput accessibilityLabel="Serie factură storno, opțional" editable={!reversing} value={stornoInvoice.series} onChangeText={(series) => setStornoInvoice((current) => ({ ...current, series }))} placeholder="Poate rămâne goală" placeholderTextColor={Colors.textMuted} selectionColor="#FB7185" cursorColor="#FB7185" autoCorrect={false} style={styles.stornoInvoiceInput} /></View><Text style={styles.stornoInvoiceHint}>Am completat seria facturii originale; o poți modifica sau șterge.</Text></View><View style={styles.stornoInvoiceField}><Text style={styles.reverseReasonLabel}>NUMĂR FACTURĂ STORNO *</Text><View style={[styles.stornoInvoiceInputWrap, stornoInvoiceTouched && !stornoInvoice.number.trim() && styles.reverseReasonInputInvalid]}><TextInput accessibilityLabel="Număr factură storno" editable={!reversing} value={stornoInvoice.number} onChangeText={(number) => setStornoInvoice((current) => ({ ...current, number }))} placeholder="Completează numărul documentului" placeholderTextColor={Colors.textMuted} selectionColor="#FB7185" cursorColor="#FB7185" autoCorrect={false} style={styles.stornoInvoiceInput} /></View>{stornoInvoiceTouched && !stornoInvoice.number.trim() && <Text style={styles.reverseReasonError}>Numărul facturii storno este obligatoriu.</Text>}</View><View style={styles.stornoInvoiceField}><Text style={styles.reverseReasonLabel}>DATA FACTURII STORNO *</Text><View style={[styles.stornoInvoiceInputWrap, stornoInvoiceTouched && !isValidIsoDate(stornoInvoice.date.trim()) && styles.reverseReasonInputInvalid]}><CalendarDays size={16} color="#FDA4AF" /><TextInput accessibilityLabel="Data facturii storno" editable={!reversing} value={stornoInvoice.date} onChangeText={(date) => setStornoInvoice((current) => ({ ...current, date }))} placeholder="AAAA-LL-ZZ" placeholderTextColor={Colors.textMuted} selectionColor="#FB7185" cursorColor="#FB7185" autoCorrect={false} maxLength={10} style={styles.stornoInvoiceInput} /></View>{stornoInvoiceTouched && !isValidIsoDate(stornoInvoice.date.trim()) && <Text style={styles.reverseReasonError}>Scrie o dată validă în formatul AAAA-LL-ZZ.</Text>}</View></View>

                <View style={styles.stornoDocuments}><View style={styles.attachmentSectionHeader}><View style={{ flex: 1 }}><Text style={styles.stornoSectionEyebrow}>DOCUMENTELE FACTURII STORNO</Text><Text style={styles.stornoSectionTitle}>{stornoAttachments.length ? `${stornoAttachments.length} ${stornoAttachments.length === 1 ? 'document pregătit' : 'documente pregătite'}` : 'PDF, imagini, Excel sau XML'}</Text></View><TouchableOpacity disabled={reversing} style={styles.stornoDocumentButton} onPress={() => void importStornoDocuments()}><FilePlus2 size={16} color="#7DD3FC" /><Text style={styles.stornoDocumentButtonText}>Alege</Text></TouchableOpacity></View>{!!stornoAttachments.length && <View style={styles.attachmentList}>{stornoAttachments.map((attachment) => <View style={[styles.attachmentCard, styles.attachmentPending]} key={attachment.key}><View style={styles.attachmentIcon}><FileDown size={16} color="#7DD3FC" /></View><View style={{ flex: 1 }}><Text style={styles.attachmentName} numberOfLines={1}>{attachment.name}</Text><Text style={styles.attachmentMeta}>Se încarcă pe NIR-ul STORNO la confirmare</Text></View><TouchableOpacity disabled={reversing} style={styles.attachmentRemove} onPress={() => setStornoAttachments((current) => current.filter((item) => item.key !== attachment.key))}><X size={15} color="#FDA4AF" /></TouchableOpacity></View>)}</View>}</View>

                <View style={styles.stornoSelectionHead}><View style={{ flex: 1 }}><Text style={styles.stornoSectionEyebrow}>POZIȚIILE DIN NIR</Text><Text style={styles.stornoSectionTitle}>{selectedStornoLines.length} din {availableStornoLines.length} selectate · {movementQuantity(selectedStornoQuantity)} unități</Text></View><TouchableOpacity accessibilityRole="button" accessibilityLabel={allStornoLinesSelected ? 'Deselectează toate produsele' : 'Selectează toate produsele'} disabled={reversing || !availableStornoLines.length} style={[styles.stornoAllButton, !availableStornoLines.length && styles.downloadDisabled]} onPress={() => { setReverseSelection(allStornoLinesSelected ? {} : Object.fromEntries(availableStornoLines.map((line) => [String(line.id), stornoLineQuantity(line).value]))); setReverseSelectionError(''); }}><Text style={styles.stornoAllButtonText}>{allStornoLinesSelected ? 'Deselectează' : 'Toate'}</Text></TouchableOpacity></View>

                <View style={styles.stornoLineList}>{availableStornoLines.map((line, index) => { const lineId = String(line.id || ''); const selected = Object.prototype.hasOwnProperty.call(reverseSelection, lineId); const maximum = stornoLineQuantity(line); return <View key={lineId} style={[styles.stornoLineCard, selected && styles.stornoLineCardSelected]}><TouchableOpacity accessibilityRole="checkbox" accessibilityState={{ checked: selected }} accessibilityLabel={`${selected ? 'Deselectează' : 'Selectează'} ${line.product_name || line.supplier_product_name || `poziția ${index + 1}`}`} disabled={reversing} activeOpacity={0.75} style={styles.stornoLineChoice} onPress={() => { setReverseSelection((current) => { const next = { ...current }; if (selected) delete next[lineId]; else next[lineId] = maximum.value; return next; }); setReverseSelectionError(''); }}><View style={[styles.stornoCheckbox, selected && styles.stornoCheckboxSelected]}>{selected && <Check size={14} color="#19090F" strokeWidth={3} />}</View><NirLineProductVisual uri={line.product_image_url} matched={Boolean(line.product_id)} /><View style={{ flex: 1, minWidth: 0 }}><Text numberOfLines={2} style={styles.stornoLineName}>{line.product_name || line.supplier_product_name || `Produs ${index + 1}`}</Text><Text numberOfLines={1} style={styles.stornoLineMeta}>Poziția {line.line_number || index + 1} · maxim {movementQuantity(maximum.numeric)} {line.purchase_unit || 'buc'}</Text></View></TouchableOpacity><View style={[styles.stornoQuantityWrap, !selected && styles.stornoQuantityDisabled]}><Text style={styles.stornoQuantityLabel}>CANTITATE</Text><TextInput accessibilityLabel={`Cantitate de stornat pentru ${line.product_name || line.supplier_product_name || `poziția ${index + 1}`}`} editable={selected && !reversing} value={selected ? reverseSelection[lineId] || '' : ''} onFocus={() => { if (!selected) setReverseSelection((current) => ({ ...current, [lineId]: maximum.value })); }} onChangeText={(value) => { setReverseSelection((current) => ({ ...current, [lineId]: value.replace(/[^0-9.,]/g, '') })); setReverseSelectionError(''); }} keyboardType="decimal-pad" selectTextOnFocus selectionColor="#FB7185" cursorColor="#FB7185" placeholder="0" placeholderTextColor={Colors.textMuted} style={styles.stornoQuantityInput} /><Text style={styles.stornoQuantityUnit}>{line.purchase_unit || 'buc'}</Text></View></View>; })}</View>
                ...
                <TouchableOpacity disabled={reversing} style={styles.deleteDialogCancel} onPress={() => { setStornoAttachments([]); setReverseDialog(false); }}><Text style={styles.deleteDialogCancelText}>Renunță</Text></TouchableOpacity><TouchableOpacity accessibilityRole="button" accessibilityLabel="Confirmă stornarea facturii" disabled={reversing || !availableStornoLines.length} style={[styles.deleteDialogConfirm, styles.reverseDialogConfirm, (!availableStornoLines.length || reversing) && styles.downloadDisabled]} onPress={() => void reverseDocument()}>{reversing ? <ActivityIndicator color="#FFFFFF" /> : <><RotateCcw size={17} color="#FFFFFF" /><Text style={styles.deleteDialogConfirmText}>Confirmă stornarea</Text></>}</TouchableOpacity>
              </ScrollView>
            </Pressable>
          </Pressable>
        </Modal>
