...
const movementLabel = (type: unknown) => ({ NIR_IN: 'INTRARE NIR', NIR_REVERSAL: 'REVERSARE NIR', SALE_OUT: 'IEȘIRE VÂNZARE', MANUAL_ADJUSTMENT: 'AJUSTARE MANUALĂ' }[String(type || '').toUpperCase()] || String(type || 'MIȘCARE STOC').replaceAll('_', ' '));
...
export default function ShopNirManager({ initialNirId = null, onInitialNirHandled }: { initialNirId?: string | null; onInitialNirHandled?: () => void }) {
  ...
  const openReverseDialog = () => {
    if (!editor || editor.status !== 'confirmed' || !can('NIR_REVERSE') || reversing) return;
    setReverseReason('');
  ...
  const reverseDocument = async () => {
    ...
    if (!token || !editor || editor.status !== 'confirmed' || reversing || !reason) return;
    setReversing(true);
    try {
      const result = await shopApi.reverseNir(token, editor.id, editor.row_version, reason);
      ...
      setReverseReasonTouched(false);
      await loadRegistry(page);
      Alert.alert('NIR reversat', `Intrarea în stoc a fost anulată prin documentul ${result.reversal.nir_number || result.reversal.temporary_number}.`);
    } catch (error) {
      Alert.alert('NIR-ul nu a fost reversat', error instanceof Error ? error.message : 'Încearcă din nou.');
    } finally {
      setReversing(false);
    }
  };
  ...
  if (editor) {
    ...
    return (
      <View style={styles.screen}>
        ...
        <Modal visible={reverseDialog} transparent animationType="fade" statusBarTranslucent onRequestClose={() => !reversing && setReverseDialog(false)}><Pressable style={styles.deleteBackdrop} onPress={() => !reversing && setReverseDialog(false)}><Pressable style={[styles.deleteDialog, styles.reverseDialog]} onPress={(event) => event.stopPropagation()}><View style={[styles.deleteDialogAccent, styles.reverseDialogAccent]} /><View style={[styles.deleteDialogIcon, styles.reverseDialogIcon]}><RotateCcw size={27} color="#FDA4AF" /></View><Text style={styles.deleteDialogEyebrow}>REVERSARE CONTABILĂ</Text><Text style={styles.deleteDialogTitle}>Reversezi această notă de intrare-recepție?</Text><Text style={styles.deleteDialogMessage}>Se creează un document separat, iar intrarea în stoc este anulată integral.</Text><View style={styles.deleteDialogDocument}><View style={{ flex: 1 }}><Text style={styles.deleteDialogMetaLabel}>DOCUMENT</Text><Text numberOfLines={1} style={styles.deleteDialogMetaValue}>{editor.nir_number || editor.temporary_number || 'NIR'}</Text></View><View style={styles.deleteDialogDivider} /><View style={{ flex: 1 }}><Text style={styles.deleteDialogMetaLabel}>FURNIZOR</Text><Text numberOfLines={1} style={styles.deleteDialogMetaValue}>{editor.supplier_name || 'Necompletat'}</Text></View></View><Text style={styles.reverseReasonLabel}>MOTIVUL REVERSĂRII *</Text><TextInput autoFocus multiline value={reverseReason} onChangeText={(value) => { setReverseReason(value); setReverseReasonTouched(false); }} editable={!reversing} maxLength={500} placeholder="Ex: document introdus din greșeală sau recepție anulată" placeholderTextColor={Colors.textMuted} selectionColor="#FB7185" cursorColor="#FB7185" style={[styles.reverseReasonInput, reverseReasonTouched && !reverseReason.trim() && styles.reverseReasonInputInvalid]} />{reverseReasonTouched && !reverseReason.trim() && <Text style={styles.reverseReasonError}>Scrie motivul reversării.</Text>}<View style={styles.deleteDialogWarning}><AlertTriangle size={18} color="#FBBF24" /><View style={{ flex: 1 }}><Text style={styles.deleteDialogWarningTitle}>Stocul trebuie să fie încă disponibil</Text><Text style={styles.deleteDialogWarningText}>Dacă marfa a fost consumată într-un document de ieșire, reversarea este blocată pentru protejarea stocului contabil.</Text></View></View><TouchableOpacity disabled={reversing} style={styles.deleteDialogCancel} onPress={() => setReverseDialog(false)}><Text style={styles.deleteDialogCancelText}>Nu, păstrează NIR-ul</Text></TouchableOpacity><TouchableOpacity disabled={reversing} style={[styles.deleteDialogConfirm, styles.reverseDialogConfirm]} onPress={() => void reverseDocument()}>{reversing ? <ActivityIndicator color="#FFFFFF" /> : <><RotateCcw size={17} color="#FFFFFF" /><Text style={styles.deleteDialogConfirmText}>Da, reversează NIR-ul</Text></>}</TouchableOpacity></Pressable></Pressable></Modal>
