  ...
  const reverseDocument = async () => {
    ...
    try {
      ...
      setStornoNotice({
        ...
        message: `${lines.length === availableStornoLines.length ? 'Toate pozițiile selectate au fost stornate' : `${lines.length} ${lines.length === 1 ? 'poziție a fost stornată' : 'poziții au fost stornate'}`} prin documentul ${result.reversal.nir_number || result.reversal.temporary_number}.`,
      });
    } catch (error) {
