...
type ReturnDecision = { order_item_id: string; product_name: string; product_sku?: string | null; requested_quantity: number; decision_status: 'pending' | 'accepted' | 'refused'; accepted_quantity: number; decision_reason: string };
...
export default function ShopOrdersManager({ initialStatusFilter = 'all', initialOrderId = null, onInitialOrderHandled, onOpenInvoice }: { initialStatusFilter?: 'all' | ShopOrder['status']; initialOrderId?: string | null; onInitialOrderHandled?: () => void; onOpenInvoice?: (invoiceId: string) => void }) {
  ...
  const save = async () => {
    ...
    if (status === 'return_confirmed' || status === 'refunded') {
      ...
      if (!returnDecisions.some((item) => item.decision_status === 'accepted' && item.accepted_quantity > 0)) {
    ...
    if (status === 'return_refused' && (returnDecisions.some((item) => item.decision_status !== 'refused') || returnDecisions.some((item) => item.decision_reason.trim().length < 3))) {
      ...
      <Modal visible={Boolean(selected)} animationType="slide" onRequestClose={() => productPanelId ? closeProductPanel() : (!saving && closeOrder())}>
        <SafeAreaView style={styles.modalSafe} edges={['top', 'bottom']}>
          ...
          {selected ? <ScrollView contentContainerStyle={[styles.modalContent, { paddingBottom: Math.max(insets.bottom, 20) + 30 }]} showsVerticalScrollIndicator={false}>
            ...
            {(['return_requested', 'return_refused', 'return_confirmed', 'refunded'] as ShopOrder['status'][]).includes(status) ? <View style={styles.returnRequestCard}><View style={styles.cancellationReasonHead}><RotateCcw size={18} color="#F472B6" /><View><Text style={styles.returnRequestKicker}>DATE RETUR</Text><Text style={styles.cancellationReasonTitle}>{selected.return_reason ? 'Datele returului' : 'Completează returul'}</Text></View></View><TextInput editable={!selected.return_reason} value={returnReason} onChangeText={setReturnReason} multiline maxLength={1000} placeholder="Motivul returului..." placeholderTextColor="#716970" style={[styles.cancellationReasonInput, !!selected.return_reason && styles.cancellationReasonInputLocked]} /><TextInput editable={!selected.return_reason} value={returnAccountHolder} onChangeText={setReturnAccountHolder} maxLength={180} placeholder="Numele titularului contului" placeholderTextColor="#716970" style={[styles.returnRequestInput, !!selected.return_reason && styles.cancellationReasonInputLocked]} /><TextInput editable={!selected.return_reason} autoCapitalize="characters" value={returnBankIban} onChangeText={(value) => setReturnBankIban(value.toUpperCase())} maxLength={40} placeholder="IBAN pentru rambursare" placeholderTextColor="#716970" style={[styles.returnRequestInput, !!selected.return_reason && styles.cancellationReasonInputLocked]} /><Text style={[styles.sectionLabel, { marginTop: 14 }]}>PRODUSE SOLICITATE · DECIZIE INDIVIDUALĂ</Text>{returnDecisions.map((item, itemIndex) => <View key={item.order_item_id} style={styles.returnDecisionCard}><View style={styles.returnDecisionHead}><View style={{ flex: 1 }}><Text style={styles.returnDecisionName}>{item.product_name}</Text><Text style={styles.returnDecisionMeta}>{item.product_sku || 'Fără SKU'} · solicitat {item.requested_quantity}</Text></View><View style={styles.returnDecisionActions}><TouchableOpacity disabled={selected.status === 'return_confirmed' || selected.status === 'refunded'} onPress={() => setReturnDecisions((current) => current.map((entry, index) => index === itemIndex ? { ...entry, decision_status: 'accepted', accepted_quantity: Math.max(1, entry.accepted_quantity || entry.requested_quantity), decision_reason: '' } : entry))} style={[styles.returnDecisionButton, item.decision_status === 'accepted' && styles.returnDecisionAccept]}><Text style={styles.returnDecisionButtonText}>Acceptă</Text></TouchableOpacity><TouchableOpacity disabled={selected.status === 'return_confirmed' || selected.status === 'refunded'} onPress={() => setReturnDecisions((current) => current.map((entry, index) => index === itemIndex ? { ...entry, decision_status: 'refused', accepted_quantity: 0 } : entry))} style={[styles.returnDecisionButton, item.decision_status === 'refused' && styles.returnDecisionRefuse]}><Text style={styles.returnDecisionButtonText}>Refuză</Text></TouchableOpacity></View></View>{item.decision_status === 'accepted' ? <View style={styles.returnQuantityRow}><Text style={styles.returnDecisionMeta}>Cantitate acceptată</Text><TouchableOpacity onPress={() => setReturnDecisions((current) => current.map((entry, index) => index === itemIndex ? { ...entry, accepted_quantity: Math.max(1, entry.accepted_quantity - 1) } : entry))} style={styles.returnQtyButton}><Text style={styles.returnDecisionButtonText}>−</Text></TouchableOpacity><Text style={styles.returnQtyValue}>{item.accepted_quantity}</Text><TouchableOpacity onPress={() => setReturnDecisions((current) => current.map((entry, index) => index === itemIndex ? { ...entry, accepted_quantity: Math.min(entry.requested_quantity, entry.accepted_quantity + 1) } : entry))} style={styles.returnQtyButton}><Text style={styles.returnDecisionButtonText}>+</Text></TouchableOpacity></View> : null}{item.decision_status === 'refused' ? <TextInput value={item.decision_reason} onChangeText={(value) => setReturnDecisions((current) => current.map((entry, index) => index === itemIndex ? { ...entry, decision_reason: value } : entry))} placeholder="Motivul refuzului pentru acest produs" placeholderTextColor="#716970" style={styles.returnRequestInput} /> : null}</View>)}<View style={styles.returnAmounts}><View><Text style={styles.returnAmountLabel}>COST RETUR CURIER</Text><Text style={styles.returnAmountCost}>−{money(Number(selected.return_shipping_cost ?? selected.configured_return_shipping_cost ?? 0))}</Text></View><View style={styles.returnAmountRight}><Text style={styles.returnAmountLabel}>ESTIMARE RESTITUIRE</Text><Text style={styles.returnAmountValue}>{money(Number(selected.return_refund_amount ?? Math.max(0, Number(selected.total || 0) - Number(selected.configured_return_shipping_cost || 0))))}</Text></View></View><Text style={styles.returnRequestHelp}>Factura de retur și NIR-ul vor include exclusiv produsele și cantitățile acceptate.</Text></View> : null}
...
const styles = StyleSheet.create({
  ...
  returnDecisionCard: { marginTop: 8, padding: 10, borderWidth: 1, borderColor: '#4E3948', borderRadius: 15, backgroundColor: '#1B161A' }, returnDecisionHead: { flexDirection: 'row', alignItems: 'center', gap: 8 }, returnDecisionName: { color: Colors.textPrimary, fontFamily: 'Inter-Bold', fontSize: 10 }, returnDecisionMeta: { color: Colors.textMuted, fontFamily: 'Inter-Regular', fontSize: 8, marginTop: 3 }, returnDecisionActions: { flexDirection: 'row', gap: 5 }, returnDecisionButton: { paddingHorizontal: 9, paddingVertical: 7, borderRadius: 10, borderWidth: 1, borderColor: '#504A51' }, returnDecisionAccept: { backgroundColor: '#123F32', borderColor: '#2DD4BF' }, returnDecisionRefuse: { backgroundColor: '#49222B', borderColor: '#FB7185' }, returnDecisionButtonText: { color: Colors.textPrimary, fontFamily: 'Inter-Bold', fontSize: 8 }, returnQuantityRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-end', gap: 8, marginTop: 9 }, returnQtyButton: { width: 28, height: 28, borderRadius: 14, alignItems: 'center', justifyContent: 'center', backgroundColor: '#343036' }, returnQtyValue: { minWidth: 24, textAlign: 'center', color: Colors.textPrimary, fontFamily: 'Inter-Bold', fontSize: 10 },
