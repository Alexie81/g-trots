...
import { ArrowLeft, BadgeCheck, Ban, BellRing, Check, ChevronRight, CircleCheckBig, Clock3, CreditCard, Download, Eye, FileCheck2, HandCoins, Mail, PackageCheck, PackageOpen, Pencil, Phone, RefreshCw, RotateCcw, Save, Search, Send, ShoppingCart, SlidersHorizontal, Star, TrendingUp, Truck, WalletCards, Warehouse, X } from 'lucide-react-native';
...
export default function ShopOrdersManager({ initialStatusFilter = 'all', initialOrderId = null, onInitialOrderHandled }: { initialStatusFilter?: 'all' | ShopOrder['status']; initialOrderId?: string | null; onInitialOrderHandled?: () => void }) {
  ...
  const shareInvoice = async (invoiceId: string, format: 'pdf' | 'xlsx' | 'xml' = 'pdf') => {
    ...
    try {
      const file = await shopApi.downloadInvoice(token, invoiceId, format);
      ...
      await Sharing.shareAsync(uri, { mimeType: file.mime_type, dialogTitle: 'Descarcă factura emisă' });
    } catch (downloadError) {
      Alert.alert('Factura nu s-a putut descărca', downloadError instanceof Error ? downloadError.message : 'Încearcă din nou.');
    ...
    if (order.invoice) {
      Alert.alert(order.invoice.display_number, `Factura este deja emisă · SPV ${order.invoice.spv_status === 'sent' ? 'trimisă' : 'netrimisă'}. Ce dorești să faci?`, [
        ...
        { text: 'Trimite pe e-mail', onPress: async () => {
          if (!token || invoiceBusy) return;
          setInvoiceBusy(order.invoice!.id);
          try {
            const result = await shopApi.sendInvoiceEmail(token, order.invoice!.id);
            Alert.alert(result.sent ? 'Factura a fost trimisă' : 'E-mail netrimis', result.sent ? `Clientul a primit factura la ${result.recipient}.` : (result.error || 'Verifică setările SMTP.'));
    ...
    <View style={styles.wrap}>
      ...
      {filtered.length ? pagedOrders.map((order) => {
        ...
        return <TouchableOpacity key={order.id} style={styles.orderCard} activeOpacity={0.76} onPress={() => void open(order)}>
          ...
          <TouchableOpacity disabled={Boolean(invoiceBusy)} style={[styles.invoiceQuick, order.invoice && styles.invoiceQuickIssued]} onPress={(event) => { event.stopPropagation(); chooseInvoiceAction(order); }}>{invoiceBusy === order.id || invoiceBusy === order.invoice?.id ? <ActivityIndicator size="small" color={order.invoice ? '#34D399' : Colors.orange} /> : order.invoice ? <Download size={17} color="#34D399" /> : <FileCheck2 size={17} color={Colors.orange} />}</TouchableOpacity>
