(function () {
  ...
  function wire() {
    ...
    $('shop-product-source').addEventListener('change', updateStockInputs);
  ...
  function renderProducts() {
    ...
    const filtered = term ? state.products.filter(product => `${product.name} ${product.sku || ''} ${product.supplier_product_code || ''} ${product.ean || ''} ${product.source_domain || ''}`.toLowerCase().includes(term)) : state.products;
  ...
  function syncOrderNotify() {
    ...
    if (helper) helper.textContent = !hasEmail ? 'Comanda nu are o adresă de e-mail.' : !changed ? 'Alege un status diferit pentru a notifica clientul.' : `Rezumatul și linkul de urmărire vor fi trimise la ${state.editingOrder.customer_email}.`;
  ...
  function renderSources() { $('shop-sources-content').innerHTML = state.sources.map(source => `<div class="shop-source-block"><article class="shop-settings-row"><span class="shop-settings-icon">${source.is_default ? '★' : '◎'}</span><div><strong>${esc(source.name)}</strong><small>${esc(source.domain)}</small><em>${source.is_active ? 'Produsele sunt vizibile pe website' : 'Produsele sunt pastrate doar in CRM'}</em></div>${source.is_default ? '<b class="commerce-pill active">IMPLICITA</b>' : ''}<label class="commerce-source-switch" title="Afiseaza sau ascunde produsele pe website"><b>${Number(source.product_count || 0)} ${Number(source.product_count || 0) === 1 ? 'produs' : 'produse'}</b><input type="checkbox" data-source-toggle="${source.id}" ${source.is_active ? 'checked' : ''}><span></span></label><button data-source-edit="${source.id}">✎</button><button class="danger" data-source-delete="${source.id}">×</button></article>${source.domain === 'boomag.ro' ? `<button type="button" class="shop-taxonomy-sync" data-source-stock-sync="${source.id}"><span>↻</span><b>Actualizeaza stocul Boomag</b><small>Stoc furnizor si stoc online din feedul zilnic</small></button><button type="button" class="shop-taxonomy-sync" data-source-taxonomy-sync="${source.id}"><span>↻</span><b>Actualizeaza structura Boomag</b><small>Categorii, subcategorii, producatori si compatibilitati · fara produse</small></button>` : ''}</div>`).join('') || empty('Nicio sursa', 'Adauga sursa produselor.'); $('shop-sources-content').querySelectorAll('[data-source-toggle]').forEach(input => input.addEventListener('change', () => toggleSourceVisibility(input.dataset.sourceToggle, input.checked, input))); $('shop-sources-content').querySelectorAll('[data-source-edit]').forEach(button => button.addEventListener('click', () => openSource(button.dataset.sourceEdit))); $('shop-sources-content').querySelectorAll('[data-source-delete]').forEach(button => button.addEventListener('click', () => deleteSource(button.dataset.sourceDelete))); $('shop-sources-content').querySelectorAll('[data-source-stock-sync]').forEach(button => button.addEventListener('click', () => syncBoomagStock(button))); $('shop-sources-content').querySelectorAll('[data-source-taxonomy-sync]').forEach(button => button.addEventListener('click', () => syncBoomagTaxonomy(button))); }
  ...
  async function syncBoomagStock(button) { if (!button || button.disabled) return; button.disabled = true; const previous = button.innerHTML; button.innerHTML = '<span>↻</span><b>Se actualizeaza stocul...</b><small>Feedul furnizorului este procesat</small>'; try { const result = await window.SHOP_API.syncBoomagStock(); toast(`${result.matched_products} produse sincronizate din ${result.feed_products} pozitii din feed.`); await loadSourcesPage(); } catch (error) { button.disabled = false; button.innerHTML = previous; toast(error.message, 'error'); } }
