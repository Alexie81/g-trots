(function () {
  ...
  async function openProduct(id = '') {
    try {
      ...
      const values = { 'shop-product-sku': product?.sku, 'shop-product-name': product?.name, 'shop-product-slug': product?.slug, 'shop-product-short': product?.short_description, 'shop-product-price': product?.price, 'shop-product-discount-value': product?.discount_value || '', 'shop-product-stock': product?.stock_quantity ?? 0, 'shop-product-low-stock': product?.low_stock_threshold ?? 3, 'shop-product-meta-title': product?.meta_title, 'shop-product-meta-description': product?.meta_description };
  ...
  async function saveProduct(event) {
    ...
    try {
      ...
      if (state.editingProduct) await window.SHOP_API.updateProduct(state.editingProduct.id, payload); else await window.SHOP_API.createProduct(payload);
      closeModal('shop-product-modal'); toast('Produsul a fost salvat.'); await loadProducts();
  ...
  async function loadPayments() { loading('shop-payments-content', 'Se incarca metodele de plata...'); try { const settings = await window.SHOP_API.getPaymentSettings(); $('shop-payments-content').innerHTML = `<form id="shop-payments-form" class="shop-settings-form"><section><span class="shop-settings-icon">▣</span><div><h2>Card online</h2><p>Permite clientilor sa selecteze plata cu cardul.</p></div><input type="checkbox" id="shop-payment-card" ${settings.card_enabled ? 'checked' : ''} /></section><label>Denumire afisata<input id="shop-payment-card-label" value="${esc(settings.card_label)}" /></label><section><span class="shop-settings-icon">⌂</span><div><h2>Ramburs la curier</h2><p>Clientul plateste la primirea coletului.</p></div><input type="checkbox" id="shop-payment-cod" ${settings.cash_on_delivery_enabled ? 'checked' : ''} /></section><label>Denumire afisata<input id="shop-payment-cod-label" value="${esc(settings.cash_on_delivery_label)}" /></label><button class="btn-primary" type="submit">Salveaza metodele de plata</button></form>`; $('shop-payments-form').addEventListener('submit', async event => { event.preventDefault(); try { await window.SHOP_API.updatePaymentSettings({ card_enabled: $('shop-payment-card').checked, cash_on_delivery_enabled: $('shop-payment-cod').checked, card_label: $('shop-payment-card-label').value.trim(), cash_on_delivery_label: $('shop-payment-cod-label').value.trim() }); toast('Metodele de plata au fost salvate.'); } catch (error) { toast(error.message, 'error'); } }); } catch (error) { failure('shop-payments-content', error); } }
