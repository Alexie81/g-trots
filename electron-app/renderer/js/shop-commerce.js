(function () {
  ...
  function updateStockInputs() { const source = state.sources.find(item => item.id === $('shop-product-source').value); const supplierManaged = String(source?.domain || '').toLowerCase() === 'boomag.ro'; const tracked = $('shop-product-stock-mode').value === 'tracked'; const supplierStock = Number(state.editingProduct?.supplier_stock_quantity || 0); const hint = $('shop-product-boomag-stock-hint'); $('shop-product-stock-mode').disabled = false; $('shop-product-stock').disabled = !tracked || supplierManaged; $('shop-product-low-stock').disabled = !tracked; $('shop-product-stock').title = supplierManaged ? `Stoc furnizor: ${supplierStock} buc. · actualizare automata zilnica` : ''; hint.hidden = !supplierManaged; hint.textContent = supplierManaged ? (tracked ? `Urmeaza automat stocul Boomag (${supplierStock} buc.).` : `Ramane disponibil indiferent de stocul Boomag (${supplierStock} buc.).`) : ''; }
