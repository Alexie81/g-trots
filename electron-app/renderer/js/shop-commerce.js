(function () {
  ...
  function openInvoiceIssue(orderId) {
    ...
    if (!order) return;
    if (order.invoice) { void downloadIssuedInvoice(order.invoice.id); return; }
  ...
  function renderOrderDetails(order) {
    ...
    $('shop-order-details').querySelectorAll('[data-order-invoice-download]').forEach(button => button.addEventListener('click', event => { event.preventDefault(); void downloadIssuedInvoice(event.currentTarget.dataset.orderInvoiceDownload, event.currentTarget.dataset.orderInvoiceFormat || 'pdf'); }));
    $('shop-order-details').querySelector('[data-order-invoice-email]')?.addEventListener('click', event => { event.preventDefault(); void emailIssuedInvoice(event.currentTarget.dataset.orderInvoiceEmail); });
