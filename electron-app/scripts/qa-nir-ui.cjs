...
app.whenReady().then(async () => {
  ...
  const setup = await win.webContents.executeJavaScript(`(() => { try {
    ...
    window.SHOP_API = {
      ...
      listProductSupplierReferences: async () => [
        ...
        { id:'nir-supplier-george', supplier_id:'supplier-george', supplier_name:'George Distribution SRL', product_id:'stock-1', supplier_product_code_original:'', supplier_product_code_normalized:'', supplier_product_name:null, supplier_ean:null, purchase_unit:'buc', stock_unit:'buc', conversion_factor:'1', is_primary_for_supplier:false, is_active:true, last_used_at:'2026-08-29', last_confirmed_purchase_price:'32', last_confirmed_currency:'RON', last_confirmed_price_ron:'32', last_confirmed_at:'2026-08-29 22:10:00', row_version:0, association_source:'confirmed_nir' },
      ],
      getProductPurchaseHistory: async () => ({ items:[], suppliers:[], statistics:{} }),
    };
  ...
  const confirmedCorrection = await win.webContents.executeJavaScript(`(async () => { const button=document.getElementById('shop-nir-correct'); const visible=Boolean(button && !button.hidden); window.confirm=()=>true; button?.click(); await new Promise(resolve=>setTimeout(resolve,180)); return { visible, calls:window.__nirQaCalls.reopen, status:document.getElementById('shop-nir-status')?.textContent, sameNumber:document.getElementById('shop-nir-title')?.textContent === 'NIR-2026-000099', supplierEditable:!document.querySelector('[data-nir-field="supplier_id"]')?.disabled }; })()`);
