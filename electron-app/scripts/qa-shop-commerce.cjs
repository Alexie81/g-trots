...
app.whenReady().then(async () => {
  ...
  await win.webContents.executeJavaScript(`(() => {
    ...
    window.SHOP_API = {
      ...
      listProducts: async () => products, getProduct: async () => products[0], getProductStats: async () => ({ product:products[0], orders_count:0, units_sold:0, revenue:0, acquisitions_total:0, profit:0, orders:[], reviews:[] }), createProduct: async value => value, updateProduct: async (_id, value) => value, deleteProduct: async () => ({ success:true, deleted_files:3 }),
  ...
  console.log(JSON.stringify({ ...metrics, errors, screenshots: { dashboardPath, productsPath, productDetailPath, editorPath, sourcesPath, orderContactPath, orderModalPath } }, null, 2));
  await win.destroy();
  app.quit();
}).catch((error) => {
  console.error(error);
