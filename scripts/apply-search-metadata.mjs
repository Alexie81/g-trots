...
<body><header class="site-header"><div class="header-inner"><a class="logo" href="/" aria-label="G-Trots, pagina principală"><img src="/assets/favicon.png" width="128" height="128" alt=""><b><span>G</span>-Trots</b></a><nav class="main-nav" aria-label="Navigație principală"><div class="mobile-nav-heading" aria-hidden="true"><span>Meniu</span><small>G-Trots</small></div><a href="/#servicii">Servicii</a><a class="nav-shop-link" href="/magazin"><span class="nav-shop-icon" aria-hidden="true"></span><span>Shop</span></a><a href="/ghiduri-service-trotinete-electrice" aria-current="page">Ghiduri</a><a href="/#proces">Cum lucrăm</a><a href="/#intrebari">Întrebări</a><a href="/contact">Contact</a></nav><a class="button button-small header-cta call-button" href="tel:+40762093915">Sună acum <span aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M6.6 10.8c1.7 3.3 3.3 4.9 6.6 6.6l2.2-2.2c.3-.3.7-.4 1.1-.3 1.2.4 2.4.6 3.6.6.6 0 .9.4.9.9V20c0 .6-.4 1-1 1C10.6 21 3 13.4 3 4c0-.6.4-1 1-1h3.6c.5 0 .9.3.9.8.1 1.3.3 2.5.6 3.6.1.4 0 .8-.3 1.1l-2.2 2.3Z"/></svg></span></a><button class="menu-toggle" type="button" aria-label="Deschide meniul" aria-expanded="false"><span></span><span></span></button></div></header>
<main class="seo-page"><article><nav aria-label="Breadcrumb"><a href="/">Acasă</a> / <a href="/ghiduri-service-trotinete-electrice">Ghiduri</a></nav><p class="eyebrow">GHID DE DIAGNOSTIC</p><h1>${escapeHtml(doc.title)}</h1><section class="quick-answer"><h2>Răspuns rapid</h2><p>${escapeHtml(doc.quick_answer)}</p></section><section><h2>Cauze posibile</h2><ul>${list(doc.likely_causes)}</ul></section><section><h2>Verificări sigure</h2><ul>${list(doc.safe_checks)}</ul></section><section><h2>Reparații posibile după diagnostic</h2><ul>${list(doc.possible_repairs)}</ul></section><section><h2>Întrebări utile pentru diagnostic</h2><ul>${list(doc.clarifying_questions)}</ul></section><p><strong>Nivel orientativ de risc:</strong> ${escapeHtml(doc.risk_level)}. Oprește utilizarea dacă vehiculul are comportament imprevizibil, miros de ars, încălzire puternică sau pierderea frânării.</p><p><a href="/contact">Contactează G-Trots pentru diagnosticare</a></p></article></main><script src="/legal-footer.js?v=20260919-whatsapp-sticky-v1" defer></script></body></html>\n`;
}
...
async function repairBrokenGuides() {
  ...
  for (const doc of documents.filter(entry => slugs.has(entry.slug))) {
    await atomicWrite(path.join(ROOT, `${doc.slug}.html`), renderGuide(doc).replace('/legal-footer.js?v=20260919-whatsapp-sticky-v1', '/legal-footer.js?v=20260919-whatsapp-sticky-v4').replace('/legal-footer.js?v=20260919-contact-beacon-v2', '/legal-footer.js?v=20260919-whatsapp-sticky-v4'));
