...
final class GtrotsInvoicePdf
{
    ...
    private static function documentHtml(array $d): string
    {
        ...
        $rows = implode('', array_map(static fn(array $item): string => self::itemRow($item, $d['currency'], $accentDark), $d['items']));
    ...
    private static function itemRow(array $item, string $currency, string $accentDark): string
    {
        $discount = $item['discount_percent'] > 0 ? '<span class="discount">-' . self::decimal($item['discount_percent'], 2) . '%</span>' : '';
        $isDelivery = mb_strtoupper(trim((string)$item['sku']), 'UTF-8') === 'TRANSPORT';
        $thumbnail = $isDelivery
            ? '<div class="product-thumb-placeholder delivery-thumb"><img src="' . self::contactIconDataUri('courier', $accentDark) . '" alt="Serviciu de livrare"></div>'
            : ($item['image'] !== null
    ...
    private static function contactIconDataUri(string $type, string $color): string
    {
        ...
        $icons = [
            ...
            'courier' => '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="' . $color . '" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6.5h10.5v10H3z"/><path d="M13.5 10h4l3.5 3.5v3h-7.5z"/><path d="M16.5 10v3.5H21"/><circle cx="7" cy="18.5" r="2"/><circle cx="17.5" cy="18.5" r="2"/><path d="M1.5 9.5H5M1.5 12.5H5"/></svg>',
        ];
