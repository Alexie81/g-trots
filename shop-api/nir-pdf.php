...
function shopNirStrictPdfHtml(array $document): string {
    ...
    $supplierName = $isCustomerReturn
        ? shopNirPdfFirst($document, ['customer_name'], shopNirPdfFirst($supplier, ['name'], 'Client'))
        : shopNirPdfFirst($supplier, ['name'], $document['supplier_name'] ?? '');
    $supplierCui = $isCustomerReturn
        ? shopNirPdfFirst($document, ['customer_email'], shopNirPdfFirst($supplier, ['email'], ''))
        : shopNirPdfFirst($supplier, ['cui', 'vat_number'], $document['supplier_cui'] ?? '');
    ...
    $documentTitle = 'NOTĂ DE RECEPȚIE ȘI<br>CONSTATARE DE DIFERENȚE';
    $documentSubtitle = $isCustomerReturn
        ? '<em class="return-subtitle">(Intrare în stoc – retur client)</em>'
        : '<em>(NIR)</em>';
    $partyLabel = $isCustomerReturn ? 'Client:' : 'Furnizor:';
    $partyIdLabel = $isCustomerReturn ? 'E-mail client:' : 'CUI furnizor:';
    $currentInvoiceLabel = $isCustomerReturn ? 'Factura de retur nr. / data:' : 'Factura furnizor nr. / data:';
    $originalInvoiceLabel = $isCustomerReturn ? 'Factura fiscală inițială nr. / data:' : 'Factura inițială stornată nr. / data:';
    $operationLabel = $isCustomerReturn ? 'Retur client' : ($isStorno ? 'Retur către furnizor (Stornare)' : 'Recepție furnizor');
    $reasonLabel = $isCustomerReturn ? 'Motiv retur:' : 'Motiv storno / retur / diferență:';
    $content = '<section class="page ' . ($isStorno ? 'storno' : 'entry') . '"><div class="document-header"><table class="header"><tr><td class="brand"><div class="brand-wrap">' . $logoHtml . '<div><div class="brand-name">G-TROTS</div><div class="company-name">' . shopNirPdfEscape($companyName) . '</div><div class="company-id">' . shopNirPdfEscape(shopNirPdfJoin([trim((string)($company['cui'] ?? '')) !== '' ? 'CUI ' . $company['cui'] : '', trim((string)($company['registration_number'] ?? '')) !== '' ? 'Reg. Com. ' . $company['registration_number'] : ''], ' | ')) . '</div></div></div></td><td class="doc-title">' . $documentTitle . $documentSubtitle . '</td><td class="form-code"><img class="form-icon" src="' . $icons['document'] . '" alt="">Cod formular:<b>NIR</b></td></tr></table>'
            ...
            . '<table class="info"><tr><td class="info-card"><table class="info-row"><tr><td class="info-label"><img class="inline-icon" src="' . $icons['user'] . '" alt="">' . $partyLabel . '</td><td>' . shopNirPdfEscape($supplierName) . '</td></tr><tr><td class="info-label"><img class="inline-icon" src="' . $icons['id'] . '" alt="">' . $partyIdLabel . '</td><td>' . shopNirPdfEscape($supplierCui) . '</td></tr><tr><td class="info-label"><img class="inline-icon" src="' . $icons['warehouse'] . '" alt="">Gestiune:</td><td>' . shopNirPdfEscape($warehouseName) . '</td></tr><tr><td class="info-label"><img class="inline-icon" src="' . $icons['pin'] . '" alt="">Loc recepție:</td><td>' . shopNirPdfEscape($receiptLocation) . '</td></tr></table></td><td class="info-card"><table class="info-row"><tr><td class="info-label"><img class="inline-icon" src="' . $icons['invoice'] . '" alt="">' . $currentInvoiceLabel . '</td><td>' . shopNirPdfEscape(shopNirPdfStrictInvoice($document)) . '</td></tr><tr><td class="info-label"><img class="inline-icon" src="' . $icons['invoice'] . '" alt="">' . $originalInvoiceLabel . '</td><td>' . shopNirPdfEscape(shopNirPdfStrictInvoice($document, true)) . '</td></tr><tr><td class="info-label"><img class="inline-icon" src="' . $icons['operation'] . '" alt="">Tip operațiune:</td><td class="' . ($isStorno ? 'storno-mark' : '') . '">' . $operationLabel . '</td></tr><tr><td class="info-label"><img class="inline-icon" src="' . $icons['info'] . '" alt="">' . $reasonLabel . '</td><td>' . shopNirPdfEscape($reason) . '</td></tr></table></td></tr></table>'
