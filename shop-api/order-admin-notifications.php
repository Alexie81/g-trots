...
function gtBuildAdminOrderNotificationEmail(array $order, array $config, string $eventType): array
{
    ...
    foreach ((array)($order['items'] ?? []) as $item) {
        ...
        $image = $imageUrl !== ''
            ? '<img src="' . gtEmailEscape($imageUrl) . '" width="76" height="76" alt="" style="display:block;width:76px;height:76px;object-fit:contain;border-radius:18px;background:#fff">'
            : '<span style="display:block;width:76px;height:76px;line-height:76px;text-align:center;border-radius:18px;background:#302d33;color:#ffb77a;font-size:19px;font-weight:900">GT</span>';
        $itemsHtml .= '<tr><td style="width:88px;padding:13px 0;border-bottom:1px solid #39353d;vertical-align:middle">' . $image . '</td>'
            . '<td style="padding:13px 12px;border-bottom:1px solid #39353d;vertical-align:middle"><strong style="display:block;color:#fff8f3;font-size:14px;line-height:1.4">' . gtEmailEscape((string)($item['product_name'] ?? 'Produs')) . '</strong>'
            . '<span style="display:block;margin-top:5px;color:#8f8790;font-size:10px">' . gtEmailEscape((string)($item['product_sku'] ?? 'Fără SKU')) . '</span>'
            . '<span style="display:block;margin-top:5px;color:#bdb5bf;font-size:11px"><strong style="color:#ffb77a">' . $quantity . ' buc.</strong> × ' . gtEmailMoney($unitPrice, $currency) . '</span></td>'
            . '<td align="right" style="padding:13px 0;border-bottom:1px solid #39353d;color:#fff8f3;font-size:13px;font-weight:900;white-space:nowrap">' . gtEmailMoney($lineTotal, $currency) . '</td></tr>';
    }
    ...
    $safeOpenUrl = gtEmailEscape(gtAdminOrderOpenUrl($order, $config));
    ...
    $html = '<!doctype html><html lang="ro"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>@media(max-width:640px){.gt-wrap{padding:10px!important}.gt-body{padding:22px 16px!important}.gt-title{font-size:31px!important}.gt-action{display:block!important;text-align:center!important}.gt-hide-mobile{display:none!important}}</style></head>'
        ...
        . '<table role="presentation" width="100%"><tr><td style="width:60px"><img src="' . $safeLogo . '" width="54" height="54" alt="G-Trots" style="display:block;border-radius:18px"></td><td><strong style="display:block;color:#fff8f3;font-size:17px">G-Trots · Comenzi</strong><span style="display:block;margin-top:4px;color:#8f8790;font-size:9px;font-weight:900;letter-spacing:.1em">NOTIFICARE OPERAȚIONALĂ</span></td><td class="gt-hide-mobile" align="right"><span style="display:inline-block;width:42px;height:42px;line-height:42px;text-align:center;border-radius:14px;background:' . $color . ';color:#171116;font-size:19px;font-weight:900">' . gtEmailEscape((string)$meta['symbol']) . '</span></td></tr></table>'
        ...
        . '<div style="padding:22px;border:1px solid #403b43;border-radius:26px;background:#151318"><table role="presentation" width="100%"><tr><td><span style="display:block;color:#8f8790;font-size:9px;font-weight:900;letter-spacing:.11em">NUMĂR COMANDĂ</span><strong style="display:block;margin-top:5px;color:#ffb77a;font-size:17px">' . $orderNumber . '</strong></td><td align="right"><span style="display:block;color:#8f8790;font-size:9px;font-weight:900;letter-spacing:.11em">DATA</span><strong style="display:block;margin-top:5px;color:#ddd6de;font-size:12px">' . $createdAt . '</strong></td></tr></table><table role="presentation" width="100%" style="margin-top:10px">' . $itemsHtml . '</table>'
        . '<table role="presentation" width="100%" style="margin-top:15px;color:#aaa2ac;font-size:12px"><tr><td style="padding:5px 0">Subtotal</td><td align="right">' . gtEmailMoney($order['subtotal'] ?? 0, $currency) . '</td></tr><tr><td style="padding:5px 0">Reducere</td><td align="right" style="color:#6ee7b7">−' . gtEmailMoney($order['discount_total'] ?? 0, $currency) . '</td></tr><tr><td style="padding:5px 0">Livrare · ' . gtEmailEscape((string)($order['shipping_method_name'] ?? 'Curier')) . '</td><td align="right">' . gtEmailMoney($order['shipping_cost'] ?? 0, $currency) . '</td></tr><tr><td style="padding:17px 0 0;border-top:1px solid #403b43;color:#fff8f3;font-size:15px;font-weight:900">TOTAL COMANDĂ</td><td align="right" style="padding:17px 0 0;border-top:1px solid #403b43;color:#ffb77a;font-size:23px;font-weight:900">' . gtEmailMoney($order['total'] ?? 0, $currency) . '</td></tr></table></div>'
        . $eventDetails
        ...
        . '<div style="padding:23px 0 5px;text-align:center"><a class="gt-action" href="' . $safeOpenUrl . '" style="display:inline-block;padding:17px 30px;border-radius:19px;background:#ff8500;color:#fff;text-decoration:none;font-size:14px;font-weight:900">Vezi comanda&nbsp;&nbsp;→</a></div><p style="margin:17px 0 0;text-align:center;color:#766f77;font-size:10px;line-height:1.55">Același buton deschide comanda în aplicația G-Trots instalată pe telefon sau pe calculator.</p>'
