...
function gtBuildAdminOrderNotificationEmail(array $order, array $config, string $eventType): array
{
    ...
    foreach ((array)($order['items'] ?? []) as $item) {
        ...
        $itemsHtml .= '<tr><td style="width:76px;padding:14px 0;border-bottom:1px solid #39353d;vertical-align:top">' . $image . '</td>'
            . '<td style="padding:14px 0 14px 12px;border-bottom:1px solid #39353d;vertical-align:top;min-width:0"><strong style="display:block;color:#fff8f3;font-size:14px;line-height:1.4;word-break:break-word;overflow-wrap:anywhere">' . gtEmailEscape((string)($item['product_name'] ?? 'Produs')) . '</strong>'
            . '<span style="display:block;margin-top:5px;color:#8f8790;font-size:10px">' . gtEmailEscape((string)($item['product_sku'] ?? 'Fără SKU')) . '</span>'
            . '<span style="display:block;margin-top:5px;color:#bdb5bf;font-size:11px"><strong style="color:#ffb77a">' . $quantity . ' buc.</strong> × ' . gtEmailMoney($unitPrice, $currency) . '</span>'
            . '<strong style="display:block;margin-top:7px;color:#fff8f3;font-size:13px;line-height:1.3">Total produs: <span style="color:#ffb77a">' . gtEmailMoney($lineTotal, $currency) . '</span></strong></td></tr>';
    }
