...
dashboardContractAssert(str_contains($api, 'WHEN m.movement_type IN ("return", "RETURN_IN") THEN -('), 'Costul FIFO al returului trebuie să inverseze costul vânzării.');
dashboardContractAssert(str_contains($api, '$grossSaleCostExpression') && str_contains($api, 'nl.line_total_ron') && str_contains($api, 'nl.stock_quantity'), 'Costul comercial al mărfii vândute trebuie reconstruit cu TVA din poziția NIR, fără a modifica FIFO-ul contabil.');
...
dashboardContractAssert(str_contains($mobileDashboard, 'Cost marfă vândută · TVA inclus') && str_contains($desktop, 'Cost marfă vândută · TVA inclus'), 'Ambele aplicații trebuie să explice că marja comercială folosește costul cu TVA inclus.');
