...
$db->exec("INSERT INTO shop_order_return_items (id, order_id, order_item_id, product_id, product_name, product_sku, requested_quantity, decision_status, accepted_quantity, unit_refund_value, line_refund_value) VALUES ('return-item-4', 'order-4', 'item-4', 'product-4', 'Produs acceptat parțial', 'GT-PARTIAL', 1, 'accepted', 1, 100, 100), ('return-item-5', 'order-4', 'item-5', 'product-5', 'Produs refuzat', 'GT-REFUSED', 1, 'refused', 0, 50, 50)");
...
invoiceAssert((int)$db->query("SELECT is_stock_item FROM shop_nir_lines l INNER JOIN shop_nir_documents d ON d.id=l.nir_document_id WHERE d.return_invoice_id='" . $onlineOnlyReturn['id'] . "'")->fetchColumn() === 0, 'Poziția documentară a returului trebuie marcată fără impact în Stocuri Conta.');
invoiceAssert((float)$db->query("SELECT inventory_cost_total_ron FROM shop_nir_documents WHERE return_invoice_id='" . $onlineOnlyReturn['id'] . "'")->fetchColumn() === 0.0, 'Returul produsului necontabil nu trebuie să inventeze un cost de stoc.');
