(() => {
  ...
  function render(order) {
    ...
    const companyRows = customerType === "PJ" ? `<p><span>Denumire firmă</span><b>${esc(order.company_name)}</b></p><p><span>CUI / CIF</span><b>${esc(order.company_cui)}</b></p><p><span>Registrul Comerțului</span><b>${esc(order.company_registration_number)}</b></p><p><span>Sediu social</span><b>${esc(order.company_address)}</b></p>` : "";
    const customerBlock = `<div class="tracking-customer-data"><header><span class="tracking-customer-badge ${customerType === "PJ" ? "is-company" : ""}">${customerType}</span><strong>${customerType === "PJ" ? "Persoană juridică" : "Persoană fizică"}</strong></header><p><span>Nume</span><b>${esc(order.customer_name)}</b></p><p><span>Telefon</span><b>${esc(order.customer_phone)}</b></p>${companyRows}<p><span>Livrare</span><b>${esc([order.address, order.city, order.county].filter(Boolean).join(", "))}</b></p></div>`;
