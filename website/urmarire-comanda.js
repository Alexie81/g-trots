(() => {
  ...
  const statuses = [
    { value: "new", label: "În procesare", description: "Comanda a intrat în fluxul de lucru.", color: "#38bdf8", icon: "⌕" },
    { value: "confirmed", label: "Confirmată", description: "Comanda și plata au fost confirmate.", color: "#34d399", icon: "✓" },
    { value: "processing", label: "În pregătire", description: "Produsele sunt pregătite pentru expediere.", color: "#fb923c", icon: "□" },
    { value: "shipped", label: "Predată curierului", description: "Pachetul a plecat către adresa de livrare.", color: "#a78bfa", icon: "→" },
    { value: "completed", label: "Livrată", description: "Comanda a ajuns la destinație.", color: "#22c55e", icon: "✓" },
    { value: "cancelled", label: "Comandă anulată", description: "Comanda nu mai este procesată.", color: "#fb7185", icon: "×" },
  ];
  ...
  function visibleStatuses(order) {
    ...
    const regular = statuses.filter(item => item.value !== "cancelled");
    const currentIndex = regular.findIndex(item => item.value === order.status);
    return order.status === "cancelled"
      ? statuses.filter(item => recorded.has(item.value) || item.value === "cancelled")
  ...
  function render(order) {
    ...
    const history = Array.isArray(order.status_history) ? order.status_history : [];
    const timeline = visibleStatuses(order).map((item, index, list) => {
      ...
      return `<article class="tracking-step ${reached ? "reached" : ""} ${isCurrent ? "current" : ""}" style="--status-color:${item.color}"><div class="tracking-step-rail"><span>${item.icon}</span>${index < list.length - 1 ? "<i></i>" : ""}</div><div class="tracking-step-copy"><header><strong>${esc(item.label)}</strong>${isCurrent ? "<span>STATUS ACTUAL</span>" : ""}</header><p>${esc(item.description)}</p>${entry ? `<time>${esc(date(entry.created_at))}</time>` : ""}</div></article>`;
    }).join("");
    ...
    resultHost.innerHTML = `<section class="tracking-progress"><header class="tracking-result-head"><div><small>COMANDA ${esc(order.order_number)}</small><h2>${esc(current.label)}</h2></div><span class="tracking-status-pill" style="--status-color:${current.color}">${esc(current.label)}</span></header><div class="tracking-timeline">${timeline}</div></section><aside class="tracking-receipt"><div class="tracking-receipt-brand"><img src="assets/logo.png" alt=""><span><strong>G-Trots România</strong><small>BON COMANDĂ</small></span></div><div class="tracking-receipt-code"><span><small>COD COMANDĂ</small><strong>${esc(order.order_number)}</strong></span><span><small>DATA</small><strong>${esc(date(order.created_at))}</strong></span></div><div class="tracking-items">${items}</div><div class="tracking-totals"><p><span>Subtotal</span><b>${money(order.subtotal, order.currency)}</b></p><p><span>Livrare · ${esc(order.shipping_method_name)}</span><b>${money(order.shipping_cost, order.currency)}</b></p><p><span>Plată</span><b>${order.payment_method === "card" ? "Card online" : "Ramburs la curier"}</b></p><p class="total"><span>Total</span><strong>${money(order.total, order.currency)}</strong></p></div><div class="tracking-help"><strong>Actualizare automată</strong>Pagina afișează direct ultimul status salvat de echipa G-Trots. Păstrează codul comenzii pentru orice întrebare.</div></aside>`;
  ...
  if (token) void load(`token=${encodeURIComponent(token)}`);
})();
